/**
 * MAE Score Result Visualization Component
 */

interface MAEScoreProps {
  result: any;
}

export function MAEScoreResult({ result }: MAEScoreProps) {
  const maeScore = result.mae_score ?? 0;
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="bg-gradient-to-br from-cyan-50 to-teal-50 border-2 border-cyan-300 rounded-xl p-8 text-center">
        ...
        <div className="inline-flex items-center gap-2 px-4 py-2 bg-white rounded-lg border border-cyan-200">
      ...
      <div className="bg-cyan-50 border border-cyan-200 rounded-lg p-6">
        <h4 className="text-md font-semibold text-cyan-900 mb-3">
          📚 What is MAE?
        </h4>
        <p className="text-sm text-cyan-800 leading-relaxed">
          Mean Absolute Error (MAE) measures the average absolute difference
          between predicted and actual values. Unlike MSE/RMSE, MAE is less
          sensitive to outliers. Lower values indicate better model performance.
        </p>
      </div>
    </div>
  );
