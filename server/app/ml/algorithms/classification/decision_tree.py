    ...
    def _extract_tree_structure(self, max_depth: int = 4) -> list:
        ...
        def traverse(node_id: int, depth: int):
            ...
            if is_leaf:
                ...
                node_info["class_label"] = (
                    str(class_names[class_idx]) if class_idx < len(class_names) else f"Class {class_idx}"
                )
                node_info["class_distribution"] = tree.value[node_id][0].tolist()
            else:
